...
CREATE FUNCTION public.__setup_exec_sql(p_sql text) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $f$ BEGIN EXECUTE p_sql; END; $f$;
CREATE FUNCTION public.__bootstrap_exec(p_sql text) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $f$ BEGIN EXECUTE p_sql; END; $f$;
CREATE FUNCTION public.__import_exec(p_sql text) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $f$ BEGIN EXECUTE p_sql; END; $f$;
