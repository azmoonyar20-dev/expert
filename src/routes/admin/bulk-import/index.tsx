    ...
    try {
      ...
      if (result.valid.length > 0) {
        ...
        for (let i = 0; i < payload.length; i += CHUNK_SIZE) {
          const res = await rpc<
            { row_number: number; is_duplicate: boolean }[] | null
          >("admin_validate_question_import", { p_rows: payload.slice(i, i + CHUNK_SIZE) });
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <Button
                variant="outline"
                className="w-full"
                onClick={() => downloadBlob(buildCsvTemplate(), "question-import-template.csv", "text/csv;charset=utf-8")}
              >
