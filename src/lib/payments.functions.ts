...
export const startZarinpalPayment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const origin =
      process.env["APP_URL"] ??
      "https://id-preview--a2f33fa0-66f0-45cf-98dc-c69eb54317b1.lovable.app";
    const callbackUrl = `${origin}${settings.callback_path || "/api/public/payments/zarinpal/callback"}`;
